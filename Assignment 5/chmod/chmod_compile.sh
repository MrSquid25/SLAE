...
#code=$(msfvenom -p linux/x86/chmod  -f c) #Shellcode generated to objdump
...
#objdump=$(objdump -d ./$code|grep '[0-9a-f]:'|grep -v 'file'|cut -f2 -d:|cut -f1-6 -d' '|tr -s ' '|tr '\t' ' '|sed 's/ $//g'|sed 's/ /\\x/g'|paste -d '' -s |sed 's/^/"/'|sed 's/$/"/g')
